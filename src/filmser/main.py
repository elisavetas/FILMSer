...
NOT_IN_ASPELL = ['albanian', 'basque', 'georgian', 'icelandic', 'japanese',
                 'kazakh', 'norwegian', 'urdu']



def main(file_path, mode="create", lang="en", spacy_size="sm",
         spell_check=False, ipa_file="", count_character=False, 
         count_bigram=False, output_type="txt|xlsx", output_dir="data/", 
         add_data_file="", stats=False, progress_bar=False):
    """
    ...
    if spell_check:
        assert lang not in NOT_IN_ASPELL, f"You have added the option of using a spell checker; however, there is no spell checker for {ISO2FULL[lang.lower()]}"
    
    # Print the chosen options
    print(f"Language: {ISO2FULL[lang.lower()]}")
    ...
    if mode == "create":
        freq_lists = create_new_list(file_path, lang=lang, spacy_size=spacy_size, 
    ...
    for data_type in freq_lists:
        ...
        file_name = f"{folder_name}{ISO2FULL[lang.lower()].lower().replace(' ', '_')}.{lang}.{data_type}.freq"
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--spacy-size", type=str, default="sm",
                            help="set the size of the spacy model used for tokenization; default: sm (small)")
